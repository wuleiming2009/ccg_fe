Page({
		...
		ccgapi
			...
			.then((resp) => {
				...
				safeSetData({
					name,
					phone,
					showWxAuth: !(wxPhone && String(wxPhone).trim()),
	...
	onReady() {
		...
		if (this._pendingUpdates && this._pendingUpdates.length) {
			const merged = this._pendingUpdates.reduce(
	...
	onShow() {
		...
		ccgapi
			...
			.then((resp) => {
				...
				const sc =
					typeof resp.send_order_count === "number"
						? resp.send_order_count
						: Number(resp.send_order_count) || 0;
				const rc =
					typeof resp.receive_order_count === "number"
						? resp.receive_order_count
						: Number(resp.receive_order_count) || 0;
				if (this._alive)
					this.setData({
						name: nm,
						initial: this.getInitial(nm),
						avatar: av,
						sentCount: sc,
						receivedCount: rc,
					});
