...
function createClient({ provider, apiKey, baseUrl, model, defaults = {} }) {
  async function chat({ messages, temperature, top_p }) {
    ...
    if (provider === 'doubao') return doubao.chat({ apiKey, baseUrl, model, messages, temperature, top_p, ...defaults })
  ...
  function chatStream({ messages, temperature, top_p, onChunk, onComplete, onError }) {
    ...
    if (provider === 'doubao') return doubao.chatStream({ apiKey, baseUrl, model, messages, temperature, top_p, ...defaults, onChunk, onComplete, onError })
    throw new Error('unsupported provider')
  }
